...
> ⚠️ `aikey auth login <provider>` 是**把 Provider 凭证加进本地 vault**；和团队版的 `aikey login`（即 `aikey account login`，连接 AiKey 控制服务）不是同一个命令，不要混。
...
aikey auth login kimi_code     # Kimi Code (api.kimi.com) — 在浏览器输入设备码
                               # ('kimi' 作为 deprecated alias 仍可用)
...
| Kimi Code (`kimi_code`) | Device Code（轮询） | 15 分钟 | Kimi Coding Plan |

> **注意**：Moonshot AI（`moonshot` provider, api.moonshot.cn）不支持 OAuth —— 请用 `aikey add` 加 API Key。
> v1.0.0-rc.2 起 Kimi 双平台拆分：`kimi(kimi-code)` 走 [api.kimi.com](https://api.kimi.com)（订阅 / OAuth），
> `kimi(moonshot)` 走 [api.moonshot.cn](https://api.moonshot.cn)（仅 API Key）。
