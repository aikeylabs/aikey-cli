...
fn resolve_binding_protocol(b: &storage::ProviderBinding) -> String {
    ...
    let resolved = match b.key_source_type {
        ...
        CredentialType::ManagedVirtualKey => {
            storage::list_virtual_key_cache_bindings(&b.key_source_ref)
                .ok()
                .and_then(|rows| {
                    let mut candidates = rows
                        .into_iter()
                        .filter(|vk| {
                            b.provider_code.is_empty()
                                || vk.provider_code.eq_ignore_ascii_case(&b.provider_code)
                                || (vk.provider_code.is_empty()
                                    && vk.supported_providers.iter().any(|provider| {
                                        provider.eq_ignore_ascii_case(&b.provider_code)
                                    }))
                        })
    ...
    // Honest fallback when the endpoint can't be resolved: only a provider with
    // one declared wire protocol can be inferred. Multi-protocol Providers must
    // remain unknown rather than being collapsed to a Provider/family label.
    resolved.unwrap_or_else(|| {
        let protocols = classifier.protocols_for_provider(&b.provider_code);
        if protocols.len() == 1 {
            protocols[0].clone()
        } else {
            "unknown".to_string()
