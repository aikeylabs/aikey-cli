...
pub fn handle_doctor_last_errors(json_mode: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = match std::env::var("AIKEY_RUN_DIR") {
        ...
        _ => match dirs::home_dir() {
            ...
            None => {
                println!("{}", "  (no home dir — cannot locate last-errors state)".dimmed());
                return Ok(());
            }
        },
    };
    ...
    let entries = parsed.get("entries").and_then(|e| e.as_array()).cloned().unwrap_or_default();
    if entries.is_empty() {
    ...
    for e in &entries {
        ...
        let upstream_id = e.get("upstream_request_id").and_then(|v| v.as_str()).unwrap_or("");

        // Top line: the request + status. Caused-by lines below name the origin.
        let status_str = format!("{status}");
        let status_col = if status >= 500 { status_str.red() } else { status_str.yellow() };
        println!("  {} {}  {}", status_col.bold(), req_path.bold(), code.dimmed());
...
fn describe_origin(origin: &str) -> String {
    use colored::Colorize;
    if origin.is_empty() {
        return "unknown (error had no origin tag — likely nginx or a pre-P1 hop)".dimmed().to_string();
    }
    if let Some(provider) = origin.strip_prefix("upstream:") {
        return format!("{} — the {} upstream returned this; not an aikey fault", origin.cyan(), provider)
            .to_string();
    }
    // component.CODE
    ...
    let hint = match component {
        ...
        "worker-proxy" => "a cluster worker produced it — check that node (routing / account / egress)",
