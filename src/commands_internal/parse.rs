...
fn run_parse_v2_rules(payload: &ParsePayload) -> Result<serde_json::Value, (&'static str, String)> {
    ...
    for d in drafts.iter_mut() {
        ...
        // v4.3 (2026-05-01): official_base_url 通过 provider_routes 单一表查询。
        // 优先按用户粘贴的 base_url 抽 host 命中行,查不到回落按 inferred_provider
        // 拿首条匹配行。返回值 = base_url + version (用户视角的 endpoint URL)。
