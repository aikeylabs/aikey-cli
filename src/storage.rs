...
pub fn get_vault_path() -> Result<PathBuf, String> {
    ...
    if let Ok(test_path) = std::env::var("AK_VAULT_PATH") {
        ...
        // If it ends with vault.db, use it directly; otherwise append DB_NAME
        if path.file_name().and_then(|n| n.to_str()) == Some(DB_NAME) {
            return Ok(path);
        } else {
...
/// Checks if the database needs migration (has old schema)
pub fn needs_migration() -> Result<bool, String> {
    ...
    // Check if id column exists (primary indicator of old schema)
...
fn migrate_database(conn: &Connection) -> Result<(), String> {
    // Check if id column exists (for very old schemas)
    let has_id: bool = conn
        .query_row(
            "SELECT COUNT(*) FROM pragma_table_info('entries') WHERE name='id'",
            [],
    ...
    if !has_id {
        ...
        // Copy data from old table to new table (only copy columns that exist)
        // Old schema only has: alias, nonce, ciphertext, created_at
        conn.execute(
    ...
    // Check if version_tag column exists
...
pub fn update_entry_full(
    ...
    conn.execute(
        "UPDATE entries
         ...
         WHERE alias = ?1",
        rusqlite::params![alias, nonce, ciphertext, version_tag, created_at, metadata],
    )
