...
# Version: 7
# Headless preflight skip (v7, 2026-07-15): mirrors hook.zsh v7 — skip the
# diagnostic probe when stdin or stderr is not a TTY, keep proxy auto-start.
# Self-sufficiency (v5, 2026-06-11): ensure the aikey binary dir is on PATH so
# this file works STANDALONE (see hook.zsh v6 note — `command aikey` searches
# PATH, fails on a fresh machine where PATH was never wired). Idempotent prepend.
case ":$PATH:" in *":$HOME/.aikey/bin:"*) ;; *) export PATH="$HOME/.aikey/bin:$PATH" ;; esac
aikey() {
    case "$1" in
        activate|deactivate)
            local _output
...
# ── Bootstrap source of active.env (v6, 2026-07-10) ───────────────────────
#
# Why this exists: PROMPT_COMMAND handles incremental sync (cheap seq-diff on
# every prompt), which is correct for the steady-state interactive REPL — but
# it never fires in shells that don't render a prompt:
#   - `bash -lc 'exec claude'`            (wrapper / scripted invocation)
#   - `bash -ilc 'source X; exec Y'`      (CI / test runner)
#   - subshells exec'd from non-bash parents
#
...
# source once at hook-source-time; PROMPT_COMMAND does ongoing updates).
# hook.zsh gained this block in v5 (bugfix 2026-05-05); bash was missed then —
# ported 2026-07-10 (systemic-fix propagation).
...
aikey_preflight() {
    ...
    # can read the probe table or answer the y/N prompt (SDK / IDE-extension
    # / CI paths; Claude Code's Bash tool replays these functions with
    # stdin=/dev/null). Keep the prompt-free proxy auto-start, skip the
    # diagnostic probe, fail open. Full rationale in hook.zsh.
    if [ ! -t 0 ] || [ ! -t 2 ]; then
