...
# Version: 6 (headless preflight skip when stdin/stderr redirected -- mirrors hook.zsh v7 -- 2026-07-15)
# v5 (2026-07-15): PS 5.1 conhost compat: UTF-8 capture for activate eval + [char]27 clear (win10-conhost-compat)
...
function global:aikey {
    ...
    if ($RemArgs.Count -ge 1 -and ($RemArgs[0] -eq 'activate' -or $RemArgs[0] -eq 'deactivate')) {
        ...
        #
        # Force UTF-8 decoding of the child's stdout while capturing. aikey
        # writes UTF-8 bytes; Windows PowerShell 5.1 decodes child output with
        # the OEM codepage (GBK on zh-CN), so a non-ASCII alias label would be
        # mojibake'd INTO the env vars we Invoke-Expression below -- breaking
        # AIKEY_ACTIVE_KEYS lookups (e.g. `aikey test <alias>`) later.
        # Scoped + restored in `finally` so the user's console is untouched.
        $extra = @($RemArgs) + @('--shell', 'powershell')
        $__aikey_prev_enc = [Console]::OutputEncoding
        try {
            [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
            $output = & $bin @extra
        } finally {
            [Console]::OutputEncoding = $__aikey_prev_enc
        }
...
function global:aikey_preflight {
    ...
    # Headless guard (2026-07-15, mirrors hook.zsh v7): stdin or stderr
    # redirected -> nobody can read the probe table or answer the y/N
    # Read-Host prompt (SDK / IDE-extension / CI invocations). Keep the
    # prompt-free proxy auto-start, skip the diagnostic probe, fail open --
    # the real CLI surfaces its own error if the credential is broken.
...
function global:aikey_clear_before_tui_handoff {
    ...
    if ($env:AIKEY_PREFLIGHT_FORCE_CLEAR -eq '1') {
        ...
        # [char]27 (not "`e"): the backtick-e escape is PowerShell 6+ only;
        # Windows PowerShell 5.1 would emit the literal text `e[H`e[2J.
        $esc = [char]27
        [Console]::Error.Write("$esc[H$esc[2J")
    }
