...
# Version: 7
# Headless preflight skip (v7, 2026-07-15): aikey_preflight skips the
# diagnostic probe when stdin or stderr is not a TTY (SDK / IDE-extension /
# CI invocations — nobody there can read the probe table or answer the y/N
# prompt). Proxy auto-start is kept. See the "Headless guard" block below.
# Self-sufficiency (v6, 2026-06-11): ensure the aikey binary dir is on PATH so
# this file works STANDALONE. The wrapper below calls `command aikey`, which
# searches PATH — on a fresh machine (no ~/.zshrc, PATH never wired) a bare
# `source ~/.aikey/hook.zsh` would otherwise make the aikey() function fail with
# "command not found: aikey" the first time it runs. Idempotent prepend.
[[ ":$PATH:" == *":$HOME/.aikey/bin:"* ]] || export PATH="$HOME/.aikey/bin:$PATH"
aikey() {
    case "$1" in
        activate|deactivate)
            local _output
...
# ── Bootstrap source of active.env (Version 5, 2026-05-05) ────────────────
#
# Why this exists: precmd handles incremental sync (cheap seq-diff on every
# prompt), which is correct for the steady-state interactive REPL — but it
# never fires in shells that don't render a prompt:
#   - `zsh -ilc 'exec claude'`            (wrapper / spike test runner)
#   - `zsh -ilc 'source X; exec Y'`       (CI / scripted invocation)
#   - subshells exec'd from non-zsh parents
#
# Without this bootstrap, those shells see empty AIKEY_ACTIVE_KEYS /
# OPENAI_API_KEY / ANTHROPIC_API_KEY etc., and aikey_preflight short-circuits
# to "no active binding" while the wrapper command runs against missing env.
#
# We mirror precmd's pin/kill-switch logic but skip the seq-diff (always
# source once at hook-source-time; precmd does ongoing incremental updates).
# Bugfix: workflow/CI/bugfix/2026-05-05-hook-active-env-not-loaded-without-precmd.md.
if [[ "$AIKEY_AUTO_REFRESH" != "off" \
   && -z "$_AIKEY_EXPLICIT_ALIAS" && -z "$AIKEY_ACTIVE_LABEL" \
   && -f ~/.aikey/active.env ]]; then
    source ~/.aikey/active.env
fi
...
aikey_preflight() {
    ...
    [[ -z "$id" ]] && return 0
    # Headless guard (v7, 2026-07-15): no TTY on stdin or stderr → nobody
    # can read the Ping/API/Chat table or answer the y/N prompt. This is
    # every SDK / IDE-extension / CI path — notably Claude Code's Bash tool
    # snapshots these wrapper functions from the user's interactive shell
    # and replays them with stdin=/dev/null. Before this guard a failed
    # probe silently aborted the wrapped CLI (`read` on closed stdin →
    # default No → exit 1), hung on an open-but-empty pipe, and every
    # invocation paid a real upstream API probe. Keep the prompt-free proxy
    # auto-start (the one preflight action headless callers need), skip the
    # diagnostic probe, and fail open — the real CLI surfaces its own error
    # if the credential is actually broken.
    if [[ ! -t 0 || ! -t 2 ]]; then
