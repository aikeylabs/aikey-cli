...
impl Core {
    ...
    pub fn resolve_environment(
        ...
        let profile = profiles::get_active_profile(conn)?
            .ok_or_else(|| "No active profile set. Use 'aikey profile use <name>' to set one.".to_string())?;

        // Get bindings for this profile
        let bindings = storage::get_profile_bindings(conn, &profile.name)?;

        // Convert bindings to a HashMap for resolution
        let mut profile_vars = HashMap::new();
        for binding_alias in bindings {
            // Try to get the secret value
            match executor::get_secret(&binding_alias, password) {
                Ok(secret_value) => {
                    profile_vars.insert(binding_alias, secret_value.to_string());
                }
                Err(_) => {
                    // Secret not found or vault error - leave it missing
                }
            }
        }
    ...
    pub fn bind_secret_to_profile(
        ...
        // Create binding
        storage::add_profile_binding(conn, profile_name, secret_alias)
    }
    ...
    pub fn unbind_secret_from_profile(
        ...
        storage::remove_profile_binding(conn, profile_name, secret_alias)
    }
    ...
    pub fn create_profile_with_bindings(
        ...
        if let Some(binding_list) = bindings {
            for alias in binding_list {
                Self::bind_secret_to_profile(conn, profile_name, &alias)?;
            }
        }
